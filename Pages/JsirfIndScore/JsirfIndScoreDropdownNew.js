...
import "./JsirfIndScoreDropdownNew.css";
...
const MenuProps = {
    autoFocus: false,
    PaperProps: {
        className: 'MenuProps3'
    },
};
const MenuProps1 = {
    PaperProps: {
        className: 'MenuProps1'
    },
};
const MenuProps2 = {
    PaperProps: {
        ...
        className: 'MenuProps2'
    },
};

// custom styled component for Paper using the styled function.
const Item = styled(Paper)(({ theme }) => ({
    backgroundColor: theme.palette.mode === 'dark' ? 'rgba(244, 244, 244, 1)' : 'rgba(244, 244, 244, 1)',
...
function MultipleSelect() {
    ...
    const [placeholderVisible3, setPlaceholderVisible3] = useState(true);

    // function triggered when Check Results button is clicked
    const handleClickDrawData = () => {
        //navigates to the /indiscoreadmin with the selected hei type, survey year and districts as state
        navigate(
            '/indiscoreadmin',
            {
                state: {
    ...
    //function triggered when survey year is selected
    const handleChange3 = (event) => {
        const selectedNames = event.target.value;
        setPersonName3(selectedNames);
        setPlaceholderVisible3(selectedNames.length === 0);
    };

    // Function to handle the deletion of the selected year (chip)
    const handleDelete3 = (value) => {
        setPersonName3((current) => _without(current, value));
        setPlaceholderVisible3(personName3.length === 1);
    };
        ...
        <>
            <div className="datavaldiv">
                ...
                <Grid container sx={{ marginTop: '10px' }}>
                    <Grid item xs={2.2} >
                        <Item className="custom-MuiPaper-root4" sx={{ backgroundColor: localStorage.getItem('theme') === 'dark' && 'rgba(60, 60, 60, 1)' }}>
                    ...
                    <Grid item xs={9.5}>
                        <Item className="custom-root4" sx={{ backgroundColor: localStorage.getItem('theme') === 'dark' && 'rgba(46, 46, 46, 1)!important' }}>
                            {personName1 && (
                                <Chip
                                    key={personName1}
                                    label={personName1}
                ...
                <Grid container>
                    <Grid item xs={2.2}>
                        <Item className="custom-MuiPaper-root5" sx={{ backgroundColor: localStorage.getItem('theme') === 'dark' && 'rgba(60, 60, 60, 1)' }}>
                            <FormControl className="formControl"
                                sx={{
                                    width: "55%",
                                    "& .MuiInputLabel-root": {
                    ...
                    <Grid item xs={9.5}>
                        <Item className="custom-root5" sx={{ backgroundColor: localStorage.getItem('theme') === 'dark' && 'rgba(46, 46, 46, 1)!important' }}>
                ...
                <Grid container>
                    <Grid item xs={2.2}>
                        <Item className="custom-MuiPaper-root6" sx={{ backgroundColor: localStorage.getItem('theme') === 'dark' && 'rgba(60, 60, 60, 1)' }}>
                            <FormControl className="formControl"
                                sx={{
                                    width: "55%",
                                    "& .MuiInputLabel-root": {
                            ...
                            >
                                <InputLabel id="demo-mutiple-chip-checkbox-label" className="dataval-text" shrink={false} >Survey Year</InputLabel>
                                <Select
                                    labelId="demo-mutiple-chip-checkbox-label"
                                    id="demo-mutiple-chip-checkbox"
                                    multiple
                                ...
                                >
                                    {name2.map((name) => (
                                        <MenuItem key={name} value={name}
                                            ...
                                            <Checkbox checked={personName3.includes(name)}
                                                style={{ color: 'black' }}
                                                size='small' />
                                            <ListItemText primary={name} />
                                        </MenuItem>
                            ...
                            <Grid>
                                {(personName1 === undefined || personName1 === null || personName2?.length === 0) ?
                                    (
                                        ...
                                        <Button onClick={() => handleClickDrawData()}
                                            style={{ color: "white", fontSize: "14px", backgroundColor: "rgba(70, 134, 85, 1)", textTransform: "none", width: "100%" }}>
                                            Check Results
                                        </Button>
                                    )}
                    ...
                    <Grid item xs={9.5}>
                        <Item className="custom-root6" sx={{ backgroundColor: localStorage.getItem('theme') === 'dark' && 'rgba(46, 46, 46, 1)!important' }}>
                            {personName3.map((value) => (
                                <Chip
                                    key={value}
                                    label={value}
                                    clickable
                            ...
                            {placeholderVisible3 && ( //displayed when survey year is not chosen
                                <div className="placeholder-textNew2" style={{color:localStorage.getItem('theme')==='dark'&&'#ffffff'}}>
                                    Choose Survey year from the dropdown
                                </div>
                            )}
                        </Item>
                    </Grid>
