...
import { IndScore_Institutions_GET_Call } from '../../services/JsirfIndScoreApi';
...
const JsirfIndScoreAdmin = () => {
    ...
    useEffect(() => {
        //API call to get the institution list with their indicative score
        IndScore_Institutions_GET_Call(state.districts, state.heitype, state.surveyYear)
            .then((response) => {
    ...
    //Function to navigate to /JsirfIndicativeTabs page when user clicks on any institution name
    //institutionName and it's aisheCode, institutionTypeIndicator, assessmentIndicator are passed as state
    ...
    return (
        <div>
            <div className='mock-content'>
                ...
                <Back />
                {/* Table component to display the indicative score of institutions */}
                    ...
                    <Table >
                        <TableHead sx={{ backgroundColor: localStorage.getItem('theme') === 'dark' ? 'rgba(141, 141, 141, 0.4)' : 'rgba(223, 223, 223, 0.5)' }}>
                            <TableRow >
                                ...
                                <TableCell className="allscoretableheader"
                                    align='left' sx={{ fontSize: `${tableheadfont}px`, paddingBottom: '4px!important', paddingTop: '22px!important', borderBottom: 'none' }}>Institution Name
                                    <Typography className='statusinfomsg' sx={{ marginLeft: '0%!important', marginTop: '0px!important' }}> *Click on institution name to view criteria-specific score</Typography></TableCell>
                                <TableCell className="allscoretableheader"
                                    sx={{
                                        borderBottom: 'none',
                        ...
                        <TableBody className="tableborder">
                            {getUniversityRes && rows.map((row, index) => (
                                ...
                                >
                                    ...
                                    <TableCell align='left' style={{ color: localStorage.getItem('theme') === 'dark' && '#ffffff', fontSize: `${tablebodyfont}px`, cursor: "pointer", borderBottom: 'none' }} onClick={() => handleClickUniversity(row.institutionName, row.aisheCode, row.institutionTypeIndicator, row.assessmentIndicator)}>{row.institutionName}</TableCell>
...
export default JsirfIndScoreAdmin
