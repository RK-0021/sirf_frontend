...
import '../../components/BackButton/Back.css'
...
function MultipleSelect() {
  ...
  useEffect(() => {
    window.scrollTo(0, 0); //Scroll the window to the top-left corner on page load
  }, []);

  // function triggered when Check Results button is clicked
  const handleClickDrawDataSample = () => {
    //navigates to the /MockSecondPage with the selected hei type and districts as state
    navigate("/MockSecondPage", {
      state: {
  ...
  return (
    <div className="datavaldiv">
      <Typography className="page-heading" sx={{ fontSize: `${pageheadfont}px`, color: localStorage.getItem('theme') === 'dark' ? '#ffffff!important' : '#12442D', marginBottom: "19px" }}>Results (Self-Improvement)</Typography>
