...
import {
  Submit_Call,
  Start_GET_Call,
  GetMockQuestionsRes_GET_Call,
} from "../../services/JsirfAssessment";
import {
...
const DataEntry = () => {
  ...
  function save_call(answer) {
    client
      .post( //POST request to save the answer
        "https://sirfqa-backend.centralindia.cloudapp.azure.com/api/assessment/jsirf/save",
      ...
      .then((response) => {
        if (response?.status === 201) {
          ...
          const isLastStep = currentStepIndex === stepNumbers.length - 1;
          if (isLastStep) {
            setIsSaved(true);
            setLoading(false);
            setIsloading(false);
          ...
          if (currentStepIndex < criteriaData.length - 1) {
            ...
            GetMockQuestionsRes_GET_Call(storedAisheCode, nextCriteriaId)
              .then((questionsRes) => {
                if (questionsRes?.status === 200) {
                  ...
                  setIsSaveDis(true);
                  setIsloading(false);
                } else if (questionsRes?.response?.status === 404) {
                  setActiveStep((prevActiveStep) => prevActiveStep + 1);
                  const isLastStep =
                    currentStepIndex + 1 === stepNumbers.length - 1;
                  if (isLastStep) {
                    setIsSaved(true);
                    setLoading(false);
                    setIsloading(false);
      ...
      .then((res) => {
        if (res.status === 200) {
          if (res.data.institutionTypeIndicator !== null) {
            {
              const institutionTypeIndicator =
              ...
              Start_GET_Call(storedAisheCode, institutionTypeIndicator)
                .then((res) => {
                  if (res.status === 200) {
                    setHasQueriedCriteria(
                      res.data.queriedCriteriaIds.length > 0
                    );
                    setQueriedCriteriaIds(res.data.queriedCriteriaIds);
                    //criteria api
                    Criteria_GET_Call(institutionTypeIndicator, "JSIRF")
                      .then((res) => {
                        if (res.status === 200) {
            ...
            res.data.institutionTypeIndicator === "AUTONOMOUS" ||
            res.data.institutionTypeIndicator === "STANDALONE INSTITUTION"
          ) {
            setIsloading(false);
            setErrorMessage(
              "Access Denied! Your Institution type is not permitted to take the assesment"
            );
            setTimeout(() => {
          ...
          else if (res.data.institutionTypeIndicator === null) { //prompting the user to select the HEI type if not selected
            setIsloading(false);
  ...
  //useEffect to set the initial values for the textfields, checkboxes, pdfs and comments
  useEffect(() => {
    ...
    const queriedPdfsName = questions
      // .filter(question => question.approvalStatus !== 'ACCEPTED')
      .map((item) => {
        if (item.fileName) {
  ...
  const handleSubmit = () => {
    ...
    //POST call to submit the assessment data
    Submit_Call(storedAisheCode, institutionTypeIndicator)
      .then((response) => {
        if (response.status === 201) {
          setShowAlert(
  ...
  const handleStepLabelClick = (stepIndex) => {
    ...
    GetMockQuestionsRes_GET_Call(storedAisheCode, selectedCriteriaId)
      .then((questionsRes) => {
        if (questionsRes?.status === 200) {
          setIsSaveDis(false);
          setQuestions(questionsRes.data);
          ...
          setUpFile([]);
          setUpFileName([]);
          const isLastStep = stepIndex === stepNumbers.length - 1;
          if (!isLastStep) {
            setIsSaved(false);
          }
        } else if (questionsRes?.response?.status === 500) {
      ...
      {isloading ? (
        ...
        <div>
          ...
          <div className="de-content">
            ...
            >
              {/* Table displaying the questions, input fields, pdfs, comments and status */}
              <TableContainer sx={{ overflow: "hidden" }}>
                <Table>
                  ...
                  <TableBody className="tableborderJDE">
                    {questions.length !== 0 &&
                      questions.map((row, index) => (
                        ...
                        >
                          ...
                          <TableCell align="left" style={{ color: localStorage.getItem('theme') === 'dark' && '#ffffff', borderBottom: 'none', fontSize: `${tablebodyfont}px` }}>
              ...
              <div>
                ...
                {isSaved ? (
                  // && currentCriteriaId === criteriaData[criteriaData.length - 1]?.id
                  <Button
                    sx={{
                      minWidth: "200px",
                      height: "7vh",
                      ...
                      <>
                        ...
                        >
                          ...
                          {/* <Button
                            ...
                            className={
                              isDisabled ? "disabledButton" : "deleteBtnJDA"
                            }
                            disabled={isDisabled}
                          >
                            Hello
