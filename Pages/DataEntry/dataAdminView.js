...
const DataAdmApprove = () => {
  ...
  useEffect(() => {
    setIsloading(true);
    //criteria api
    Criteria_GET_Call(state.instType, "MOCK")
      .then((res) => {
        if (res.status === 200) {
  ...
  //function to handle the click event on the step label
  const handleStepLabelClick = (stepIndex) => {
    ...
    GetAdminQuestions_GET_Call(selectedCriteriaId) //fetching questions based on the selected criteria
      .then((response) => {
        setIsloading(false);
      ...
      {isloading ? (
        ...
        <div>
          ...
          <div className="de-content2">
            ...
            >
              {/* Table displaying questions and the input fields */}
              <TableContainer>
                <Table>
                  <TableHead
                  ...
                  <TableBody className="tableborderDEA">
                    {questions?.length !== 0 &&
                      questions?.map((row, index) => (
                        ...
                        >
                          <TableCell
                            ...
                            scope="row"
                            className="de-rowDA"
                            style={{ color: localStorage.getItem('theme') === 'dark' && '#ffffff', borderBottom: 'none', fontSize: `${tablebodyfont}px` }}
                          >
                            {`${index + 1}.`}
                          </TableCell>
                          <TableCell align="left" className="de-rowDA" style={{ color: localStorage.getItem('theme') === 'dark' && '#ffffff', fontSize: `${tablebodyfont}px`, borderBottom: 'none' }}>
                            {row.question.questionDescription}
                          </TableCell>
                          <TableCell style={{ borderBottom: 'none', fontSize: `${tablebodyfont}px` }}>
